...
Second, division by 0 cannot be defined. Starting from division, we might or might not know that it is defined from multiplication: $$\frac{a}{b}$$ means the UNIQUE number $$c$$ s.t. $$cb=a$$. If $$b=0, a!=0$$: no $$c$$ exists, since $$c*0=0!=a$$. If $$a,b=0$$, then $$c$$ is not unique, because any $$c$$ is valid for $$c*0=0$$.
